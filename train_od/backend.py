...
class FullYoloFeature(BaseFeatureExtractor):
    ...
    def __init__(self, input_width, input_height):
        input_image = Input(shape=(input_width, input_height, 3))
...
# for each layer, channel number divided by 4
class TinyYoloFeature_1(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height, input_channel):
        input_image = Input(shape=(input_height, input_width, input_channel))
...
class TinyYoloFeature_2(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height, input_channel):
        input_image = Input(shape=(input_height, input_width, input_channel))
...
class TinyYoloFeature_3(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height, input_channel):
        input_image = Input(shape=(input_height, input_width, input_channel))
...
class TinyYoloFeature_4(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height, input_channel):
        input_image = Input(shape=(input_height, input_width, input_channel))
        ...
        # Layer 7
        x = Conv2D(128, (2,2), strides=(1,1), padding='same', name='conv_' + str(i+7), use_bias=False)(x)
        x = BatchNormalization(name='norm_' + str(i+7))(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class TinyYoloFeature_5(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height, input_channel):
        input_image = Input(shape=(input_height, input_width, input_channel))
        ...
        # Layer 7
        x = Conv2D(64, (2,2), strides=(1,1), padding='same', name='conv_' + str(i+7), use_bias=False)(x)
        x = BatchNormalization(name='norm_' + str(i+7))(x)
        x = LeakyReLU(alpha=0.1)(x)
...
class Inception3Feature(BaseFeatureExtractor):
    ...
    def __init__(self, input_width, input_height):
        ...
        inception = InceptionV3(input_shape=(input_width,input_height,3), include_top=False)
...
class VGG16Feature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height):
        vgg16 = VGG16(input_shape=(input_width, input_height, 3), include_top=False)
...
class ResNet50Feature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height):
        resnet50 = ResNet50(input_shape=(input_width, input_height, 3), include_top=False)
...
class MyYoloFeature(BaseFeatureExtractor):
    """docstring for ClassName"""
    def __init__(self, input_width, input_height):
        input_image = Input(shape=(input_width, input_height, 3))

        # Layer 1
        x = Conv2D(16, (3,3), strides=(2,2), padding='same', name='conv_1', use_bias=False)(input_image)
        x = BatchNormalization(name='norm_1')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 2
        x = Conv2D(32, (1,1), strides=(1,1), padding='valid', name='conv_2', use_bias=False)(x)
        x = BatchNormalization(name='norm_2')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 3
        x = Conv2D(32, (3,3), strides=(2,2), padding='same', name='conv_3', use_bias=False)(x)
        x = BatchNormalization(name='norm_3')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 4
        x = Conv2D(64, (1,1), strides=(1,1), padding='valid', name='conv_4', use_bias=False)(x)
        x = BatchNormalization(name='norm_4')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 5
        x = Conv2D(64, (3,3), strides=(2,2), padding='same', name='conv_5', use_bias=False)(x)
        x = BatchNormalization(name='norm_5')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 6
        x = Conv2D(128, (1,1), strides=(1,1), padding='valid', name='conv_6', use_bias=False)(x)
        x = BatchNormalization(name='norm_6')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 7
        x = Conv2D(128, (3,3), strides=(2,2), padding='same', name='conv_7', use_bias=False)(x)
        x = BatchNormalization(name='norm_7')(x)
        x = LeakyReLU(alpha=0.1)(x)

        # Layer 8
        x = Conv2D(256, (1,1), strides=(1,1), padding='valid', name='conv_8', use_bias=False)(x)
        x = BatchNormalization(name='norm_8')(x)
        x = LeakyReLU(alpha=0.1)(x)
        ...
        # Layer 10
        x = Conv2D(256, (1,1), strides=(1,1), padding='valid', name='conv_10', use_bias=False)(x)
        x = BatchNormalization(name='norm_10')(x)
        x = LeakyReLU(alpha=0.1)(x)
